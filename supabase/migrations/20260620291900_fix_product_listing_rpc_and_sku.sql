-- PostgREST sends p_step as integer; accept INTEGER in the RPC signature.
-- Issue 14-digit numeric SKUs and replace legacy DRAFT-* placeholders.
...
BEGIN
  ...
  RETURN LPAD(yr::TEXT, 2, '0') || LPAD(next_counter::TEXT, 12, '0');
END;
$$;

CREATE OR REPLACE FUNCTION public.product_listing_needs_sku(p_sku TEXT)
RETURNS BOOLEAN
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT COALESCE(BTRIM(p_sku), '') = ''
    OR BTRIM(p_sku) LIKE 'DRAFT-%'
    OR BTRIM(p_sku) !~ '^\d{14}$';
$$;

DROP FUNCTION IF EXISTS public.save_seller_product_listing_draft(BIGINT, SMALLINT, BOOLEAN, JSONB);
...
AS $$
BEGIN
  PERFORM public.save_seller_product_listing_draft(p_product_id, 5, true, p_payload);
...
REVOKE ALL ON FUNCTION public.product_listing_needs_sku(TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.save_seller_product_listing_draft(BIGINT, INTEGER, BOOLEAN, JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.submit_seller_product_listing(BIGINT, JSONB) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.save_seller_product_listing_draft(BIGINT, INTEGER, BOOLEAN, JSONB) TO authenticated;
