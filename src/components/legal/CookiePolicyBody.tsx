...
function PolicyBlock({ block }: { block: CookiePolicyBlock }) {
  if (block.kind === 'ul') {
    return (
  ...
  if (block.kind === 'h3') {
    return <h3 className="seller-agreement-document__subsection-title">{block.text}</h3>
  }
  ...
  return (
    <div className="seller-agreement-document__body">
      <header className="seller-agreement-document__intro">
        {cookiePolicyIntro.map((block, index) => (
